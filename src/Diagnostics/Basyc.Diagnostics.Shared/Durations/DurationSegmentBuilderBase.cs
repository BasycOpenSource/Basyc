...
public abstract class DurationSegmentBuilderBase : IDisposable, IDurationSegmentBuilder
{
    ...
    /// <summary>
    /// Gets service (application) executing this segment.
    /// </summary>
    ...
    {
        bool wasStarted = EnsureStarted(out var rootStartTime);
        return StartNested(service, segmentName, wasStarted ? DateTimeOffset.UtcNow : rootStartTime);
    }

    public virtual IDurationSegmentBuilder StartNested(string segmentName, DateTimeOffset start) => StartNested(Service, segmentName, start);

    public virtual IDurationSegmentBuilder StartNested(string segmentName) => StartNested(Service, segmentName);
