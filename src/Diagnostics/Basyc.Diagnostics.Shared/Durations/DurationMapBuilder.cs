...
public class DurationMapBuilder : IDurationMapBuilder
{
    ...
    public IDurationSegmentBuilder StartNewSegment(string segmentName, DateTimeOffset startTime) => StartNewSegment(Service, segmentName, startTime);

    public void End() => End(DateTimeOffset.UtcNow);
    ...
    public ValueTask Log(string message, LogLevel logLevel) => throw new NotImplementedException();

    public void Dispose()
    {
        if (rootSegmentBuilder is not null)
        {
            var endTime = rootSegmentBuilder.End();
            End(endTime);
        }

        End();
        rootSegmentBuilder?.Dispose();
    }
}
