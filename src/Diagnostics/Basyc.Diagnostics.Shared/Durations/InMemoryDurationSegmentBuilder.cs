...
{
    private readonly List<InMemoryDurationSegmentBuilder> nestedSegmentBuilders = new();
    ...
    {
        bool wasStarted = EnsureStarted(out var starTtime);
    ...
    public override ValueTask Log(string message, LogLevel logLevel) => throw new NotImplementedException();

    private DurationSegment BuildNestedSegments()
    {
        var nestedSegments = new DurationSegment[nestedSegmentBuilders.Count];
        for (int nestedSegmentIndex = 0; nestedSegmentIndex < nestedSegmentBuilders.Count; nestedSegmentIndex++)
        {
            var nestedSegmentBuilder = nestedSegmentBuilders[nestedSegmentIndex];
