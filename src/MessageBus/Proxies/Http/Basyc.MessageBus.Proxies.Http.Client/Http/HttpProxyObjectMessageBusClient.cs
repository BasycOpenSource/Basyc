...
public class HttpProxyObjectMessageBusClient : IObjectMessageBusClient
{
    ...
    public BusTask PublishAsync(string eventType, RequestContext requestContext = default, CancellationToken cancellationToken = default) => HttpCallToProxyServer(eventType, null, null, cancellationToken).ToBusTask();

    public BusTask PublishAsync(string eventType, object eventData, RequestContext requestContext = default, CancellationToken cancellationToken = default) => HttpCallToProxyServer(eventType, eventData, null, cancellationToken).ToBusTask();

    public BusTask SendAsync(string commandType, RequestContext requestContext = default, CancellationToken cancellationToken = default) => HttpCallToProxyServer(commandType, null, null, cancellationToken).ToBusTask();

    public BusTask SendAsync(string commandType, object commandData, RequestContext requestContext = default, CancellationToken cancellationToken = default) => HttpCallToProxyServer(commandType, commandData, null, cancellationToken).ToBusTask();
    ...
    {
        var innerBusTask = HttpCallToProxyServer(requestType, null, typeof(UnknownResponseType), cancellationToken);
    ...
    {
        var innerBusTask = HttpCallToProxyServer(requestType, requestData, typeof(UnknownResponseType), cancellationToken);
        return innerBusTask.ContinueWith<object>(x => x);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            var httpErrorContent = httpResult.Content.ReadAsStringAsync(cancellationToken).GetAwaiter().GetResult();
        ...
        httpResult.Content.CopyToAsync(httpMemomoryStream, cancellationToken).GetAwaiter().GetResult();
