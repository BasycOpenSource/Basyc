...
public static class ReceiveExtension
{
    ...
    public static Task Receive<TServerCanCall, TMethodToWait>(
        IStrongTypedHubConnectionReceiver<TServerCanCall> strongTypedHubConnection,
        Expression<Func<TServerCanCall, TMethodToWait>> methodSelector,
        TMethodToWait handler)
    ...
    {
        var subscriptionId = (Connection: connection, Id: id);
        actionToSubscriptionMap[subscriptionId].Dispose();
        actionToSubscriptionMap.Remove(subscriptionId);
    }

    private static MethodInfo GetMethodFromSelector<TServerCanCall, TMethodToWait>(Expression<Func<TServerCanCall, TMethodToWait>> methodSelector)
        where TServerCanCall : notnull
    {
        ...
        where TLambda : Delegate
    {
        ...
        throw new InvalidOperationException("Unknown delegate");
    }

    private static void CheckArgumentTypes<TLambda>(TLambda lambda, object?[] arguments)
        where TLambda : Delegate
    {
        var lambdaParamInfos = lambda.Method.GetParameters();
        ...
        {
            var lambdaParamInfo = lambdaParamInfos[paramIndex];
            object? argument = arguments[paramIndex];
            if (argument is null)
            {
                if (lambdaParamInfo.ParameterType.CanBeNull() is false)
                {
                    throw new InvalidOperationException("argument cant be null");
                }
            }
            else
            {
                ...
                {
                    throw new InvalidOperationException($"Provided argument on index {paramIndex} does not match delegate paramter {argType.Name}");
                }
            }
        }
    }
