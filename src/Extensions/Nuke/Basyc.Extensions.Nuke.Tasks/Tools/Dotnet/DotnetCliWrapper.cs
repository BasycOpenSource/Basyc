...
{
    public static bool FormatVerifyNoChanges(string workingDirectory,
        string project,
        IEnumerable<string> filesToCheck,
        out DotnetFormatReport report,
        out ProcessException? processException)
    {
        ...
        try
        {
            ...
            string include = filesToCheck.Any() ? $" --include {includeParam}" : string.Empty;
            ...
            report = new(Array.Empty<ReportRecord>());
        }
    ...
    public static void NugetSignWithFile(IEnumerable<string> packagesPaths, string certPath, string? certPassword) => DotNet(
