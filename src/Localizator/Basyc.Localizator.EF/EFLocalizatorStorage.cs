...
public class EfLocalizatorStorage : ILocalizatorStorage
{
	...
	{
		var model = new Abstraction.Localizator(new CultureInfo(entity.CultureName), entity.SectionUniqueName, entity.Values);
		return model;
	}

	private LocalizatorEntity LocalizatorToEntity(ILocalizator model)
	{
		var entity = new LocalizatorEntity(model.Culture.Name, model.SectionUniqueName, new Dictionary<string, string>(model.GetAll()),
			new LocalizatorSectionEntity(model.SectionUniqueName));
		return entity;
	}
