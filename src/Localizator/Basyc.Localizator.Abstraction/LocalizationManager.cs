...
public class LocalizationManager : ILocalizationManager
{
    ...
    public LocalizationManager(
        ...
        IOptions<LocalizationManagerOptions> managerOptions,
        ILocalizatorStorage storage)
    {
        ...
        this.storage.StorageChanged += (s, a) =>
        {
            ...
            {
                bool oldSectionExists = sections.TryGetValue(changedSectionName, out var oldSection);
    ...
    public IDictionary<string, ILocalizatedSection> GetSections() => sections;

    public ILocalizatedSection GetSection(string sectionName)
    {
        if (TryGetSection(sectionName, out var section) == false)
        {
            throw new InvalidOperationException($"Section with key \"{sectionName}\" does not exist");
        }
    ...
    public Task SaveOrUpdateLocalizators(params ILocalizator[] localizators) => storage.SaveOrUpdateLocalizatorsAsync(localizators);
    ...
    public void ChangeDefaultSectionCulture(string sectionName, CultureInfo newCulture)
    {
        ...
        {
            throw new InvalidOperationException($"Can't change section default culture. Section with name {sectionName} does not exists");
        }
    }

    private void SectionCultureChangedHandler(object? sender, SectionCultureChangedEventArgs a)
    {
        sender.ThrowIfNull();
    ...
    private void OnSectionCultureChanged(string sectionName, CultureInfo oldCulture, CultureInfo newCulture) => SectionCultureChanged?.Invoke(this, new SectionCultureChangedEventArgs(sectionName, oldCulture, newCulture));
}
