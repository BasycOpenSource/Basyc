...
{
    public static async Task ReceiveNumberAsync(int number)
    {
        await Task.Delay(150);
        throw MethodExceptionHelperException.CreateForCurrentMethod(new object?[] { number });
    }

    public void ReceiveNumber(int number) => throw MethodExceptionHelperException.CreateForCurrentMethod();
